...
require("dotenv").config();
const express = require("express");
const User = require("../model/user.js"); // Ensure this path correctly points to your user model
const { verifyWebhook } = require("@clerk/backend/webhooks");
...
router.post("/", async (req, res) => {
  ...
  try {
    ...
    // POSTMAN TESTING BYPASS: Check if you passed the custom test header
    ...
    // 1. Handle User Created or Updated
    if (evt.type === "user.created" || evt.type === "user.updated") {
      const u = evt.data;
      console.log("👤 Processing user data for Clerk ID:", u.id);
      ...
      console.log("🎉 User successfully synced in DB. Object ID:", savedUser._id);
    }

    // 2. Handle User Deleted
    if (evt.type === "user.deleted") {
      if (evt.data.id) {
        await User.findOneAndDelete({ clerkId: evt.data.id });
        console.log("🗑️ User deleted from DB:", evt.data.id);
